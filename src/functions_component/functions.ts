 ...
 */
export async function calculateStationaryCarbonFootprintParams(
  date: string,
  country: string,
  stateProvince: string,
  fuelName: string,
 ...
 */
export async function calculateStationaryCarbonFootprintRow(row: any[][]): Promise<string[][]> {
  try {
    const flat = Array.isArray(row) && Array.isArray(row[0]) ? row[0] : row;
    ...
    return await calculateStationaryCarbonFootprintParams(
      String(date),
      String(country),
      String(stateProvince),
      String(fuelName),
